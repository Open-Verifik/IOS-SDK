    ...
    func getCredentials(vc: VerifikProtocol, token: String,
        ...
        let task = session.dataTask(with: request as URLRequest, completionHandler: { data, response, error in
            ...
            guard let data = data else {
                ...
                vc.configError(error: "Verifik error: Couldn't get Verifik credentials")
                return
            }
            if let responseJSONObj = try? JSONSerialization.jsonObject(with: data, options: JSONSerialization.ReadingOptions.allowFragments) as? [String: AnyObject],
               let responseJSONData = try? JSONSerialization.jsonObject(with: Data((responseJSONObj["data"] as? String)?.utf8 ?? "".utf8), options: JSONSerialization.ReadingOptions.allowFragments) as? [AnyObject]{
                ...
                */
                if deviceKeyIdentifier != nil, publicFaceScanEncryptionKey != nil {
                    ...
                    vc.configError(error: "Verifik error: Credentials with wrong format, please contact Verifik Support Team")
                }
            }
        })
        task.resume()
    ...
    func getSessionToken(vc: VerifikProtocol, token: String,
        ...
        let task = session.dataTask(with: request as URLRequest, completionHandler: { data, response, error in
            ...
            guard let data = data else {
                ...
                vc.sessionError(error: "Verifik Error: Exception raised while getting session data.")
                return
            }
            if let responseJSONObj = try? JSONSerialization.jsonObject(with: data, options: JSONSerialization.ReadingOptions.allowFragments) as? [String: AnyObject] {
                if let sessionToken = responseJSONObj["data"] as? String{
                    ...
                    vc.sessionError(error: "Verifik Error: Exception raised while setting session data.")
                }
            }
        })
        task.resume()
