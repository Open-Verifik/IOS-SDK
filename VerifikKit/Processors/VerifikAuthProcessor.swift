...
class VerifikAuthProcessor: NSObject, Processor, FaceTecFaceScanProcessorDelegate, URLSessionTaskDelegate {
    ...
    func processSessionWhileFaceTecSDKWaits(sessionResult: FaceTecSessionResult, faceScanResultCallback: FaceTecFaceScanResultCallback) {
        ...
        if sessionResult.status != FaceTecSessionStatus.sessionCompletedSuccessfully {
            ...
            self.vp.authError(error: "User cancel authentication or there was a connection error")
        ...
        latestNetworkRequest = session.dataTask(with: request as URLRequest, completionHandler: { data, response, error in
            ...
            guard let data = data else {
                ...
                self.vp.authError(error: "There was an error parsing authentication resulting data, please contact Verifik Support Team")
                faceScanResultCallback.onFaceScanResultCancel()
                return
            }
            ...
            guard let responseJSON = try? JSONSerialization.jsonObject(with: data, options: JSONSerialization.ReadingOptions.allowFragments) as! [String: AnyObject] else {
                // CASE:  UNEXPECTED response from API.  Our Sample Code keys off a wasProcessed boolean on the root of the JSON object --> You define your own API contracts with yourself and may choose to do something different here based on the error.
                self.vp.authError(error: "There was an error parsing authentication resulting data 2, please contact Verifik Support Team")
                faceScanResultCallback.onFaceScanResultCancel()
                return
            }
            ...
            if let code = responseJSON["code"] as? String,
                code == "NotFound"{
                self.vp.authError(error: "User not found on database")
                faceScanResultCallback.onFaceScanResultCancel()
                return
            }
            if let code = responseJSON["code"] as? String,
                code == "Conflict"{
                self.vp.authError(error: "Biometric is disabled")
                faceScanResultCallback.onFaceScanResultCancel()
                return
            }
            if let code = responseJSON["code"] as? String,
                let message = responseJSON["message"] as? String{
                self.vp.authError(error: "\(code) \(message)")
                faceScanResultCallback.onFaceScanResultCancel()
                return
            }
            ...
            guard let scanResultBlob = responseJSON["data"]?["scanResultBlob"] as? String,
                ...
                // CASE:  UNEXPECTED response from API.  Our Sample Code keys off a wasProcessed boolean on the root of the JSON object --> You define your own API contracts with yourself and may choose to do something different here based on the error.
                self.vp.authError(error: "There was an error with the auth process")
                faceScanResultCallback.onFaceScanResultCancel()
                return;
            }
            ...
            if wasProcessed == true {
                ...
                // CASE:  UNEXPECTED response from API.  Our Sample Code keys off a wasProcessed boolean on the root of the JSON object --> You define your own API contracts with yourself and may choose to do something different here based on the error.
                self.vp.authError(error: "No authorization granted")
                faceScanResultCallback.onFaceScanResultCancel()
                return;
            }
        })
    ...
    func onFaceTecSDKCompletelyDone() {
        ...
        self.vp.onAuthDone(done: success)
    }
